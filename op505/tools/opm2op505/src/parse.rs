//! 由来: ym38x6/tools/opm2x6/src/parse.rs（コミット8d9c6bb時点の複製、2026-08-11）。
//! ym38x6依存排除（デフォーク）に伴う複製。以後は独立に進化させる（fork-on-write）。
//! opm2x6側の修正は自動では反映されない（`git diff 8d9c6bb -- ym38x6/tools/opm2x6/src/parse.rs`
//! で追従漏れを確認できる）。`sample.opm`（テスト用固定フィクスチャ）もリポジトリ内複製として
//! `op505/tools/opm2op505/sample.opm`へ複製した。
