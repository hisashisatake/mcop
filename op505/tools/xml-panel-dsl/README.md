...
## ウィンドウを狭めたときの挙動（2026-08-22追加: 最小幅の下げ止まり＋横スクロール）

`<row>`/`<stack>`のレイアウト計算（taffy）は`flex_shrink: 0.0`を明示している
（`ui/layout/src/lib.rs`）。ウィンドウ幅を縮めても中のノブ/`<time-eg-editor>`等は
**縮まず**、収まりきらない分はコンテナからあふれる。生成される描画関数の先頭には
`panel.xml`全体から算出した`PANEL_MIN_WIDTH`定数（`ui-codegen`の`Layout::min_full_width`）が
出力され、`full_width = ui.available_width().max(PANEL_MIN_WIDTH)`でこの値を下回らないよう
下げ止める。ルートの`ScrollArea`も`vertical()`から`both()`へ変更済みで、ウィンドウが
`PANEL_MIN_WIDTH`より狭いときはあふれた分を横スクロールで見せる。

（旧実装は`flex_shrink`が既定の1.0のままで、taffyが計算した縮小後の矩形と、ウィジェットが
実際に描画する宣言サイズが食い違い、ノブ等が`<time-eg-editor>`へ視覚的に重なって見える
不具合があった。`flex_shrink: 0.0`＋`PANEL_MIN_WIDTH`下げ止めの組み合わせで、
「縮めて重ねる」のではなく「縮めずにあふれさせてスクロールさせる」設計に統一した。）

ホスト側（`op505-vst`のResizableWindow最小サイズ、`gesture-app`のTauriウィンドウ`minWidth`）も
`PANEL_MIN_WIDTH`相当の値以上に設定してあるため、通常の操作ではスクロールバーが出る前に
リサイズの下限へ到達する。ただし`gesture-app/src-tauri/tauri.conf.json`の`minWidth`は
（VSTの`PANEL_MIN_WIDTH`参照と違い）静的なハードコード値なので、panel.xmlの内容を
大きく変えたときはこの値も見直すこと。
...
### `span`（幅の配分、12カラムグリッド or パーセント指定、2026-08-22改訂）

同じ`<panels>`内の`<panel>`は、幅の配分を`span`属性で指定する。内部表現は常に
0〜1の割合（`Panel::span_fraction`）で、`span="N"`（整数）と`span="N%"`（パーセント）の
どちらの記法で書いても同じ割合へ解決される。

- `span="N"`（1〜12の整数）… CSSでおなじみの12分割グリッド（Bootstrap等のcol-span相当）。
  12分の`N`の幅を占める。**同じ`<panels>`内の全`<panel>`をこの記法で揃えた場合、
  span合計はちょうど12でなければならない**（書き漏れ等のミスをパースエラーで検出するため。
  例: TEXTURE LFO(7)+FILTER(5)=12）。
- `span="N%"`（0〜100の数値＋`%`）… 直接の割合指定。12の約数に縛られない配分
  （例: `span="25%"`+`span="75%"`）ができる。**同じ`<panels>`内の全`<panel>`をこの記法で
  揃えた場合、合計は100%でなければならない**（端数丸め対策として±0.05%までは許容、
  `33.34%`+`33.33%`+`33.34%`のような組み合わせが通る）。
- **整数指定とパーセント指定は同じ`<panels>`内で混在できない**（どちらかに揃える。
  グループが違えば混在してよい）。
- `span`を**全`<panel>`で省略**すると要素数で均等割りする（1個なら100%、2個なら50%+50%、
  5個なら20%×5）。12で割り切れるかは問わない（旧実装は5分割等をエラーにしていたが、
  `span_fraction`化に伴い撤廃した）。**一部の`<panel>`だけ省略するのは不可**
  （曖昧さを避けるため、明示するなら全部・省略するなら全部、のどちらか）。
