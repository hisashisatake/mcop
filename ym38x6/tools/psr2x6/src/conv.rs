...
/// 3dB/step+15で-93dBジャンプ）→ 38x6（サスティンレベル 0=ほぼ無音, 255=フルレベル）。
///
/// **【2026-07-15 dB線形へ修正】** 旧実装は`(15-sl)*17`という**レジスタ段差の線形写像**だった
/// （極性自体はOPQのslが既に「減衰量」極性で正しかったが、写像カーブが誤り）。
...
/// エンジンがdBリニア化された直後（2c654a9の前日）に書かれたこのファイルが追従できていなかった。
/// opm2x6と同じdB線形式に統一する。旧コメントが参照していた`sl_to_level`は既にエンジンから
/// 撤去済みの関数で、根拠として無効だった。
#[inline]
...
/// あったが、opzref4x6(ymfm実機参照)基準の再測定で天井なしの方が実機に近いと判明し既定を撤廃した
/// （[[project_opz2x6_d1l_polarity_bug]]混入時の聴感判定だった疑いが根拠。詳細はspec-fm.md参照）。
/// PSR-70にはopzref4x6相当の実機参照レンダラーが無く同様の定量検証はできていないが、仕様の
...
mod tests {
    ...
    fn sl_mid_values_follow_db_linear_curve_not_stair_step() {
        ...
        // 新実装はopm2x6と同じ3dB/step+15dBジャンプのdB線形カーブに従う。
        assert_eq!(sl_opq_to_x6(4), 222); // -12dB → 255*(1-12/93)
        assert_eq!(sl_opq_to_x6(8), 189); // -24dB → 255*(1-24/93)
        assert_eq!(sl_opq_to_x6(14), 140); // -42dB → 255*(1-42/93)
        // 旧実装の値(187/119)とは異なることを明示（端点だけのテストでは検出できなかった回帰）
        assert_ne!(sl_opq_to_x6(4), 187);
        assert_ne!(sl_opq_to_x6(8), 119);
    }
