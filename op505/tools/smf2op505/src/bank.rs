...
//! （SMF再生では音色が見つからず無音になると曲が丸ごと落ちるため、`Op505PresetBank::get`の
//! `None`をそのまま伝えず、この階層でフォールバックを維持する。
//! `ym38x6/tools/smf2wav/src/bank.rs`と同形、fork-on-write）。

use op505_core::{Op505Patch, Op505PresetFile};

/// 128 プログラム分のパッチテーブル。
pub struct PatchBank {
    table: Box<[Op505Patch; 128]>,
}

impl PatchBank {
    /// `(program, patch)` の列から構築する。`program` は 0〜127（範囲外は破棄）。
    /// 同一 program が複数あれば後勝ち。空の場合はエラー。
    pub fn from_entries(entries: Vec<(u8, Op505Patch)>) -> Result<Self, String> {
        let mut slots: [Option<Op505Patch>; 128] = std::array::from_fn(|_| None);
        ...
        let mut last: Option<Op505Patch> = None;
        let table: Box<[Op505Patch; 128]> = Box::new(std::array::from_fn(|i| match &slots[i] {
            Some(p) => {
                last = Some(*p);
                *p
            }
            None => last.unwrap_or_else(|| slots[first].expect("first は定義済み")),
        }));
        Ok(Self { table })
    }

    /// `.op505`（`Op505PresetFile`）から構築する。`Presets`/`Programs` どちらの形式でも
    /// 各エントリーの `program` 番号でパッチを配置する（ファイルが宣言する`bank`番号は無視し、
    /// ファイル全体を単一の128プログラムテーブルとして扱う）。
    pub fn from_preset_file(file: &Op505PresetFile) -> Result<Self, String> {
        let entries = match file {
            Op505PresetFile::Presets { presets, .. } => presets,
            Op505PresetFile::Programs { programs, .. } => programs,
        };
        let list: Vec<(u8, Op505Patch)> = entries.iter().map(|e| (e.program, e.patch)).collect();
        Self::from_entries(list)
    }

    /// 連番のパッチ列から構築する（変換ツールの「ボイス番号＝プログラム番号」用）。
    /// `patches[i]` をプログラム番号 `i` に割り当てる（128 件目以降は破棄）。
    pub fn from_patches(patches: &[Op505Patch]) -> Result<Self, String> {
        let list: Vec<(u8, Op505Patch)> =
            patches.iter().take(128).enumerate().map(|(i, p)| (i as u8, *p)).collect();
        Self::from_entries(list)
    }

    /// プログラム番号（下位 7bit）に対応するパッチを返す。
    pub fn patch(&self, program: u8) -> &Op505Patch {
...
mod tests {
    ...
    fn marked(fb: u8) -> Op505Patch {
        let mut p = Op505Patch::default();
