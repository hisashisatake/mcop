...
import { PROGRESSIONS, matchProgressions } from '../src/progressions.ts';
import type { NormalizedFamily } from '../src/types.ts';

const KNOWN_FAMILIES = new Set<NormalizedFamily>(['maj', 'dom', 'min', 'halfdim', 'dim']);
    ...
    for (const step of prog.steps) {
      ...
      if (step.suffixes) {
        ...
        for (const f of step.families!) {
          assert.ok(KNOWN_FAMILIES.has(f as NormalizedFamily), `${prog.id}: unknown family "${f}"`);
        }
      }
    }
  }
...
test('12小節ブルース: minMatch=4により、反復構造による途中からの誤検出が起きない', () => {
  ...
  const twoI = [
    { degree: 0, normFamily: 'dom' as const, suffix: '7' },
    { degree: 0, normFamily: 'dom' as const, suffix: '7' },
  ];
  ...
  const midFour = [
    { degree: 5, normFamily: 'dom' as const, suffix: '7' },
    { degree: 5, normFamily: 'dom' as const, suffix: '7' },
    { degree: 0, normFamily: 'dom' as const, suffix: '7' },
    { degree: 0, normFamily: 'dom' as const, suffix: '7' },
  ];
...
test('カノン進行: 2手一致で次の一手が返り、1手だけでは返らない(minMatch=2)', () => {
  ...
  const oneChord = [{ degree: 0, normFamily: 'maj' as const, suffix: '' }];
  const oneMatch = matchProgressions(oneChord, 'major');
  assert.ok(!oneMatch.some((m) => m.id === 'canon'), '1手だけではマッチしないはず');

  const twoChords = [
    { degree: 0, normFamily: 'maj' as const, suffix: '' },
    { degree: 7, normFamily: 'dom' as const, suffix: '7' },
  ];
...
test('カノン進行: 末尾(V)まで一致し、非cyclicなので次の一手は無い', () => {
  const full = [
    { degree: 0, normFamily: 'maj' as const, suffix: '' },
    { degree: 7, normFamily: 'dom' as const, suffix: '7' },
    { degree: 9, normFamily: 'min' as const, suffix: 'm' },
    { degree: 4, normFamily: 'min' as const, suffix: 'm' },
    { degree: 5, normFamily: 'maj' as const, suffix: '' },
    { degree: 0, normFamily: 'maj' as const, suffix: '' },
    { degree: 5, normFamily: 'maj' as const, suffix: '' },
    { degree: 7, normFamily: 'dom' as const, suffix: '7' },
  ];
...
test('line-cliche: suffix完全一致で判定される(familiesでは区別できないケース)', () => {
  ...
  const tail = [
    { degree: 0, normFamily: 'min' as const, suffix: 'm' },
    { degree: 0, normFamily: 'min' as const, suffix: 'mMaj7' },
  ];
...
test('Just the Two of Us進行はminMatch=1のため1手目だけでもマッチする', () => {
  ...
  const oneChord = [{ degree: 5, normFamily: 'maj' as const, suffix: 'maj7' }];
...
test('Just the Two of Us進行のminMatch=1は先頭ステップ限定。途中のii(度数2)を単独/2手目で弾いても誤って進捗表示されない', () => {
  ...
  const iim9Only = [{ degree: 2, normFamily: 'min' as const, suffix: 'm9' }];
  ...
  const imaj9ThenIim9 = [
    { degree: 0, normFamily: 'maj' as const, suffix: 'maj9' },
    { degree: 2, normFamily: 'min' as const, suffix: 'm9' },
  ];
...
test('maxResultsで結果件数が絞られる', () => {
  ...
  const fourChords = [
    { degree: 0, normFamily: 'maj' as const, suffix: '' },
    { degree: 7, normFamily: 'maj' as const, suffix: '' },
    { degree: 9, normFamily: 'min' as const, suffix: '' },
    { degree: 5, normFamily: 'maj' as const, suffix: '' },
  ];
