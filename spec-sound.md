...
**op505向けには2026-08-25に移植済み**: 下記の音色構成（Algorithm 7・OP1のみ可聴・ピアノ/リード
2 ADSR）を踏襲しつつ、op505では3点変更した。

1. 「特殊バンクを実行時コードでフォールバック生成する」パターン自体を廃止しているため、
   `op505/tools/patchlab/python/waveform_memory_bank.py`が実体の`.op505`ファイル（bank=16383）を
   作る方式にした。GM2ドラムキット（`gm2_drum_kit.py`）と同じ方式で、op505-core/op505-vst/
   gesture-appのコード変更は不要（詳細はCLAUDE.md「プロジェクト概要」参照）。
2. **波形をym38x6版の基本4波形のみから、op505ビルトイン波形32種すべてへ拡大した**
   （ユーザー指定）。下記「ビルトイン波形（スロット0〜31）」節がそのままop505版にも当てはまる
   （名前の割り当ては`waveform_memory_bank.py`の`WAVE_NAMES`参照）。
3. **Program番号の並びを変更した**。ym38x6版はピアノ/リードを4個ずつ交互に置く`program % 8`
   だったが、op505版は**Program 0〜31＝ピアノ風ADSR・波形0〜31、32〜63＝リード風ADSR・波形0〜31**
...
**（ym38x6版のみ）** 波形メモリ音色（1オペレーター）は素直なオシレーターとして使うため、
ビルトイン32種のうち**基本4波形（sine=0 / saw=8 / square=16 / triangle=24、各基本波の
variant0）のみ**を提供する（OPZ8変換やPWMバリエーションは含めない）。これを ピアノ風 /
リード風 の2 ADSR と組み合わせ、Program 0〜7 に配置する（8以降は同じ並びを繰り返す =
`program % 8`）。**op505版はビルトイン32種すべてを使う**（上記「位置づけ」節の変更点2・3参照、
Program 0〜31＝ピアノ風・波形0〜31／32〜63＝リード風・波形0〜31、重複なし。Program 64〜127は
使わない）。
