...
これらとは別の並行トラックとして、後継チップ**フェーズ12：OP505**が2026-08-10時点でコア実装・
gesture-app配線・エディタUIまで一通り完了している（詳細はフェーズ12参照）。
...
フェーズ5: 実機音色資産の取り込みと音作り基盤（進行中）
  ...
  → feedback上限1.8（ハイハット系のノイズ再現）とOPNベース音色（音程安定性）のトレードオフは解決済み:
    エンジンのフィードバック帰還を2サンプル平均へ既定化し、opz2x6/psr2x6のmod_tl_cap既定を
    None（天井なし）化することで両方を改善（2026-07-17〜18、`ea2b998`）。詳細はmemory参照
...
フェーズ12: OP505（後継チップ、N点Time/Level方式EG）（コア実装〜gesture-app配線まで完了）
  ...
  → `op505/core`（クレート名op505-core）を新設し、EG系統（オペレーターEG・Pitch/Cutoff/Gain
    ...
    チップ内LFO・質感LFO）は`sound-fm`/`ym38x6-core`をそのまま共有する。既存`.38x6`パッチは
    Adapterでキーオン部を厳密変換・リリースは公称値変換
  ...
  → スコープ外（未着手）: `.op505`ファイルI/O、op505-vst（VST3/CLAPプラグイン）
```
