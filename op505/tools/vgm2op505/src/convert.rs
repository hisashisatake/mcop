//! OP505直接変換のPatchConverter実装。CLI（main.rs）とゴールデンテスト
...
//! `opm2x6`/`mucom2x6`型固定シグネチャへの橋渡し関数）は、`crate::patch::PatchConverter`が
//! これらの型を直接要求するようになったため不要になった。

use op505_core::Op505Patch;

use crate::patch::PatchConverter;
...
impl PatchConverter for Op505Converter {
    ...
    fn from_opm(&mut self, v: &opm2op505::parse::OpmVoice) -> (Op505Patch, Vec<String>) {
        opm2op505::conv::voice_to_op505_patch(v, opm2op505::parse::OperatorOrder::Direct, self.attack.to_opm())
    }
    fn from_opn(&mut self, v: &mucom2op505::map::OpnVoice) -> (Op505Patch, Vec<String>) {
        mucom2op505::conv::voice_to_op505_patch(v, self.attack.to_mucom())
    }
    fn from_fixed(&mut self, p: Op505Patch) -> (Op505Patch, Vec<String>) {
        (p, Vec::new())
    }
}
