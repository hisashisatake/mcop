...
2. **変更を実装**する。
3. **リビルド**して `smf2op505_opt.exe` として別名保存する。`git stash`/`Copy-Item`でソース
   ファイルを行き来させてbaseline/opt両方を作る場合は、ビルド前に必ず
   `cargo clean -p <該当クレート> --release`でキャッシュを明示的にクリアすること
   （Cargoのfingerprintキャッシュがファイル内容の変更を見逃し、古いバイナリをそのまま
   返すことがある。ビルド時間が普段より極端に短い（数秒→0.1秒台）ときは要注意）。
   2つのexeを作り終えたら**必ず`Get-FileHash`で両者が異なることを確認してから**
   計測に入る（同一だったら比較そのものが無意味）。
...
- **baseline/optのexeが偶然同一ハッシュになる事故が実際に起きた**（2026-09-11、env_ampキャッシュ改善の検証時）。`Copy-Item`でソースを書き戻した直後のビルドが異常に短時間で完了し、生成された2つのexeが完全に同じSHA256だった状態でA/B計測してしまい、「差がない（むしろ僅かに遅い）」という誤った結果を得た。原因は上記手順3の`cargo clean`未実施。詳細は`feedback_cargo_build_cache_false_negative`メモリ参照。
