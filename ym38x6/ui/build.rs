//! `src/panel.xml`（正本）から`draw_param_panel`本体を生成し、`panel.rs`の
//! `include!(concat!(env!("OUT_DIR"), "/panel_generated.rs"))`が読み込む
//! `$OUT_DIR/panel_generated.rs`へ書き出す。
//!
//! パース・コード生成のロジックは`panel-codegen`（egui非依存の純粋クレート）に一本化されており、
//! `tools/xml-panel-dsl`のブラウザプレビュー（wasm）とも共有する。ここでは
//! ファイルI/Oと失敗時のビルド停止のみを担う。
...
fn main() {
    ...
    let rust = panel_codegen::generate_rust(&xml)
