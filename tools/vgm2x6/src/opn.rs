...
//! 音色変換は mucom2x6 の `conv::OpnVoice::to_ym38x6_patch()` を再利用する。

use mucom2x6::conv::{OpnOperator, OpnVoice};

// ---------------------------------------------------------------------------
// FMサンプルあたりの master clock 数（= F-Number→周波数式の分母 factor）
// ---------------------------------------------------------------------------
//
// freq = fnum * chip_clock / (FM_DIVISOR * 2^(20-block))
//
// F-Number式の分母係数 144 は全OPNチップ共通（YM2203/YM2608/YM2612 全て同一）。
// 「3ch vs 6ch」の違いはサンプルレート(clock/72 vs clock/144)に現れるが、
// F-Number→周波数の計算式には影響しない。
// 実機検証済み: YM2203(4MHz)でYs PC-88曲をA=440基準で確認。
pub const FM_DIVISOR_6CH: u32 = 144;
