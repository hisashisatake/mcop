...
`.claude/skills/`（プロジェクト内、audition/fm-compare/phrase/perf-bench）はドキュメント兼Claude参照用、
実際にスラッシュコマンドとして使うには`~/.claude/skills/<name>/SKILL.md`へのコピーが必要。
`/scope`・`/handoff`・`/handoff-restore`・`/gui-probe`はプロジェクトコード非依存の汎用スキルのため
`~/.claude/skills/`にのみ定義を置きリポジトリにはコミットしない。各スキルの使い方・概要は
実行時のスキル一覧（description）を参照。

```powershell
# 初回セットアップ（プロジェクト内スキルのスラッシュコマンド化）
...
### リリースビルド（VST3/CLAP・standalone/MMEドライバ・NSISインストーラ）

op505-vstのVST3/CLAPバンドル生成、op505-standalone/MMEドライバのインストール、NSIS統合
インストーラ（op505-setup.exe）のビルド手順は`/release-build`スキル参照（`.claude/skills/release-build.md`）。
