...
// 聞こえているかの代表値）→ (level+1)%4 → マスの先頭パルスだけを新レベルで書き込み、
// 残りは無音(0)にする（`setRhythmRange`を1回）。1マス=1ヒットという意味を保つための設計:
// Rust側`clock_loop`は非0の生パルスを見るたびnote_on/offを発火するため、マス幅ぶん
// （粗い倍率では最大24パルス）全部を同じ非0値で埋めると、そのマスを鳴らすたびに
// 生パルス数だけ高速に連打され「ワンショットの音が細かい音符の連続に聞こえる」不具合になる
// （2026-09-17実機確認・修正）。`cellLevel()`は範囲内の最大値を読むため、この「先頭だけ
// 書き込み」でも見た目（そのマスが塗られているかどうかの表示）は従来と変わらない。
//
...
export function setupRhythmScreen(canvas: HTMLCanvasElement): { draw: (ctx: CanvasRenderingContext2D) => void } {
  canvas.addEventListener('mousedown', (e) => {
    ...
    pushUndo();
    // マスの先頭パルスだけを新レベルで書き込み、残りは無音にする（1マス=1ヒット。
    // モジュールdoc冒頭の「クリック処理」節参照。範囲内にあった細かい打ち込みは
    // 消音を含めまとめて統一される点は従来どおり）。
    row.steps[cell.step] = next;
    for (let i = 1; i < snap; i++) row.steps[cell.step + i] = 0;
