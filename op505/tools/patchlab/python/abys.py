...
実行モード:
  ...
  [2] バッチ推論（GM2 WAV → .op505）:
      python abys.py --input-dir private/gm2_wav --out private/bank0_abys.op505
...
def load_ref_patches(path: Path) -> list[dict]:
    """ref.38x6 ファイル（レート方式の旧ym38x6スキーマ）から presets リストを返す。
    各要素: {program, name, patch}。patch_json_to_vector()がレート方式dict専用のため、
    参照バンクは常に.38x6形式のまま（op505の.op505形式では読めない、意図的な制限）。"""
    import json
    import re
...
def main() -> int:
    ...
    ap.add_argument("--ref-patch", type=str, default=None,
                    help="参照 .38x6 ファイルパス（レート方式の旧ym38x6スキーマ。.op505は非対応）。"
                         "マッチしたパッチを CMA-ES 初期値候補に追加する")
    ...
    if args.target_desc:
        ...
        out_path = Path(args.out) if args.out else Path("private/target_desc_out.op505")
    ...
    if args.input_dir:
        ...
        default_out = wav_dir.parent / "bank0_abys.op505"
