...
**op505向けには2026-08-25に移植済み**: 下記の音色構成をそのまま踏襲しつつ、op505では
「特殊バンクを実行時コードでフォールバック生成する」パターン自体を廃止しているため、
`op505/tools/patchlab/python/waveform_memory_bank.py`が実体の`.op505`ファイル（bank=16383、
Program 0〜127を静的に生成）を作る方式に変更した。GM2ドラムキット（`gm2_drum_kit.py`）と同じ方式で、
op505-core/op505-vst/gesture-appのコード変更は不要（詳細はCLAUDE.md「プロジェクト概要」参照）。
