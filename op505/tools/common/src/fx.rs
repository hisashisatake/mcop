//! マスターリバーブの後段適用。
//!
//! DAW（REAPER 等）で `.op505` を鳴らすときと同じく、ドライ出力に
//! `MasterEffects` のリバーブセンドを後段で掛けて聴感を再現するためのヘルパー。
//! `send`=0 のときは何もしない（＝従来どおりの完全ドライ）。
//! `ym38x6/tools/smf2wav`の`fx.rs`と同形（fork-on-write、詳細はCargo.tomlのコメント参照）。

use sound_core::{AudioProcessor, MasterEffects, ReverbType};
