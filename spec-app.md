...
**チャンネルLFOとの関係:**
∞ジェスチャーによる装飾・ビブラートはF-Numberの直接制御（連続更新）であり、チャンネルLFOは使わない。
ゆっくり移動によるポルタメントはCC65/CC5を介した合成エンジン側のグライドで実現するため、こちらもLFOとは無関係。
チャンネルLFO（LFO1/LFO2）はジェスチャーとは独立した自動ビブラート/トレモロ等の別用途に使用可能。
なお、一般的なモジュレーションホイール（CC1）は③ジェスチャー層としてチャンネルLFO1のDepthへ瞬間加算する経路だが、本アプリはホイールを用いず∞ジェスチャーのF-Number直接制御を採るため、両者は別系統である。
