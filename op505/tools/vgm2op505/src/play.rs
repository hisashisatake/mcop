...
fn eval_ssg_channel<C: PatchConverter>(
    ...
    let idx = if tone_active && noise_on {
        bank.find_or_insert_fixed(mix_patch(noise_per), SsgPatchId::Mix(noise_per), "ssg_mix")
    } else if tone_active {
        bank.find_or_insert_fixed(psg_patch(), SsgPatchId::Psg, "psg_square")
    } else {
        bank.find_or_insert_fixed(noise_patch(noise_per), SsgPatchId::Noise(noise_per), "ssg_noise")
    };
    let patch = bank.patch_at(idx);
