...
//!
//! 由来: ym38x6/tools/opz2x6/src/parse.rs（コミット8d9c6bb時点の複製、2026-08-11）。
//! ym38x6依存排除（デフォーク）に伴う複製。以後は独立に進化させる（fork-on-write）。
//! opz2x6側の修正は自動では反映されない（`git diff 8d9c6bb -- ym38x6/tools/opz2x6/src/parse.rs`
//! で追従漏れを確認できる）。
//!
//! 対応フォーマット:
...
mod tests {
    ...
    fn vmem_grandpiano_voice1_spot_check() {
        ...
        let syx = std::fs::read(
            "../../../ym38x6/tools/opz2x6/private/extracted/Yamaha Factory Bank A.syx"
        );
        // ファイルが存在しない環境ではスキップ
